...
Frame::Frame (uint8_t src, uint8_t addr, short data) {
...
char Frame::CRC8 (const char* data, int length) 
{
   char crc = 0x00;
   char extract;
   char sum;
...
bool Frame::is_valid (void) {
	return checksum == frame [3];
}

Header::Header (uint8_t dest, uint8_t length) {
